    ...
    <div class="contacts-list">
        <ul>
            ...
            <li v-for="contact in sortedContacts" :key="contact.id" @click="selectContact(contact)" :class="{ 'selected': contact == selected }">
                <div class="avatar">
                    <img :src="`${contact.avatar.length > 0 ? '/cabinet/storage/images/smalls/100x100/' + contact.avatar[0].name : '/cabinet/images/avatar.png'}`" class="rounded-circle" alt="Изоображение">
                </div>
                {{contact.online}}
                <div class="contact">
                    <p class="name">{{ contact.settings.name != undefined ? contact.settings.name : 'Имя не указано'}}</p>
                </div>
