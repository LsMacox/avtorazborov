    ...
    <div class="content">
        <section class="enotification">
            ...
            <form class="enotification__steps" id="form_enotification" method="post" action="#">
                ...
                <button class="enotification__button" id="enotification__button" onclick="save_alert_fetch_data()">Создать рассылку<i class="far fa-envelope"></i></button>
...
var $users = @json((object)$users);
$users.forEach(function (user) {
    users[user.id] = user.shop_setting.name;
});
    ...
    inputValidator: function (value) {
        return new Promise(function (resolve, reject) {
            if (value !== '') {
                ...
                resolve('Вы не выбрали пользователя');
            }
        });
    },
}).then(function(result) {
    user_id = result.value;
    getCars(laroute.route('admin.mail-list.transport.available', {user_id: user_id}),
        laroute.route('transport.cars.get-marks'),
        'popupModels(1, this, "car")');
    form_transport['user_id'] = user_id;
});
...
async function renderModels () {
    ...
    axios({url: laroute.route('admin.mail-list.transport.available', {user_id: user_id}), method: 'post', data: $mark})
        .then(available => {
...
function save_transport_fetch_data() {
    axios({
        url: '{{route('admin.mail-list.transport.store')}}',
        method: 'post',
        data: {transport: form_transport, user_id: user_id}
    })
    .then(response => {
        helpers.popup(-1);
...
function save_alert_fetch_data() {
    ...
    if (!is_synonym) {
        ...
        axios({
            url: '{{route('admin.mail-list.store')}}',
            method: 'post',
            data: {
                user_id: user_id,
                email: form_email,
        ...
        .then(response => {
            location.href = '{{ route('admin.mail-list.index') }}';
        })
        .catch(error => {
            let errors = '';
