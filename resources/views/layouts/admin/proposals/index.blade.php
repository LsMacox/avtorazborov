        ...
        <div class="table-responsive">
            <table class="table table-main table-striped table-hover table-proposal" style="font-size: 14px;">
                ...
                @forelse($proposals as $proposal)
                    <tr onclick="document.location.href = '{{ route('admin.proposal.show', $proposal->id) }}'">
                        ...
                        <td scope="col" class="text-center align-middle pt-1 pb-1">
                            <a href="{{route('admin.proposal.edit', $proposal->id)}}" class="text-secondary mr-1 ml-1" style="font-size: 22px;" title="Редактировать" onclick="event.stopPropagation();">
                                <i class="fa fa-edit" style="font-weight: 400"></i>
                            </a>
                            <a href="#" class="text-secondary" style="font-size: 22px;" onclick="event.stopPropagation(); if(confirm('Вы ' +'дейстивтельно хотите удалить ' +'заявку?')) {event.preventDefault(); document.getElementById('proposal_del-{{$proposal->id}}').submit();}" title="Удалить">
                                <i class="fas fa-trash-alt" style="font-weight: 400"></i>
                            </a>
                            <form action="{{ route('admin.proposal.destroy', $proposal->id) }}" method="POST" id="proposal_del-{{$proposal->id}}">
                                @csrf
                                @method('DELETE')
                            </form>
                        </td>
                ...
                @empty
                    <tr>
                        ...
                        <td class="text-muted text-center" style="font-size: 25px" colspan="11">Скоро здесь появятся заявки</td>
                    </tr>
                @endforelse
                </tbody>
            </table>
...
function addProposals(data) {
    ...
    function simpleTemplating(data) {
        ...
        $.each(data, function(index, item){
            ...
            html +=  `
                                      <td scope="col" class="text-center align-middle pt-1 pb-1">
                                          <a href="${ laroute.route('admin.proposal.edit', {proposal: item.id}) }" class="text-secondary mr-1 ml-1"
                                              style="font-size: 22px;" title="Редактировать" onclick="event.stopPropagation();">
                                              <i class="fa fa-edit" style="font-weight: 400"></i>
                                          </a>
                                          <a
                                          href="${ laroute.route('admin.proposal.destroy', {proposal: item.id}) }"
                                          class="text-secondary  mr-1 ml-1"
                                          style="font-size: 22px;"
                                          title="Удалить"
                                          onclick="event.stopPropagation(); if(confirm('Вы дейстивтельно хотите удалить ' + 'заявку?')) { document.getElementById('proposal-delete-${item.id}').submit() }"
                                          >
                                              <i class="fas fa-trash-alt" style="font-weight: 400"></i>
                                          </a>
                                          <form action="#" method="post" class="d-none" id="proposal-delete-${item.id}">
                                                @csrf
            @method('DELETE')
            </form>
        </td>`;
