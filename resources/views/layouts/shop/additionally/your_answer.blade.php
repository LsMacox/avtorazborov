...
@section('content')
    <div class="content-header">
        ...
        <div class="row mb-2 d-flex justify-content-center">
          <div class="col-sm-6">
            <p class="m-0 text-dark text-center proposal_p">Ваши ответы</p>
          </div>
        </div>
      </div>
    </div>
            ...
            <table class="table table-main table-striped table-hover table-proposal" style="font-size: 14px;" id="proposals">
                ...
                <tbody id="data-container">
                  @forelse($proposals as $proposal)
                        <tr style="cursor: pointer;" id="{{ $proposal->id }}" onclick="document.location.href = '{{ route('shop.proposal.show',$proposal->id) }}'">
                          @if ( count( App\Models\Message::where('proposal_id', $proposal->id)->where('to', auth()->id())->where('read', false)->get() ) > 0 )
                              <td class="bg-white p-1 text-center">
                                  <i class="fas fa-certificate" style="color: #f6bf00; font-size: 30px;"></i>
                              </td>
                          @else
                            <td class="bg-white">
                            </td>
